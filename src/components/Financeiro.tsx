...
const Financeiro: React.FC = () => {
  const { sabor, brasa, brilho, brilhoClients, brilhoExpenses, conecta, bencao, participants, loading } = useConsolidatedData();
  ...
  const campFees = participants.map(p => {
    let price = 0;
    if (p.paymentType === 'Inteira') price = 120;
    if (p.paymentType === 'Meia') price = 60;
    return { ...p, price };
  });
    ...
    <div className="space-y-6 pb-20 md:pt-20 px-4">
      <header className="py-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 font-medium">Consolidado Acampa Central</p>
        </div>
